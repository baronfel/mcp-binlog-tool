...
public class SearchTasksTool
{
    ...
    [Description("Find all invocations of a specific MSBuild task across all projects (e.g., 'Csc', 'Copy') and return execution summary. Returns a dictionary of dictionaries - the outer dictionary is keyed by project id, the inner keyed by target id.")]
        ...
        [Description("The name of the task to search for (case-insensitive)")] string taskName)
    {
        ...
        {
            var projectResults = new Dictionary<int, Dictionary<int, SimpleTaskInfo>>();
            foreach (var target in project.Children.OfType<Target>())
            {
                ...
                var targetResults = new Dictionary<int, SimpleTaskInfo>();
                projectResults[project.Id] = targetResults;
                foreach (var task in matchingTasks)
                {
                    targetResults[task.Id] = new (task.Name, (long)(task.EndTime - task.StartTime).TotalMilliseconds);
                }
            }
        }
